...
//内存管理参数	   
const u32 memtblsize[SRAMBANK]={MEM3_ALLOC_TABLE_SIZE};	//内存表大小
const u32 memblksize[SRAMBANK]={MEM3_BLOCK_SIZE};		//内存分块大小
const u32 memsize[SRAMBANK]={MEM3_MAX_SIZE};			//内存总大小
...
{
    return mymalloc(SRAMCCM,xWantedSize);
}
...
{
    return myrealloc(SRAMCCM, pv, size);
}

void pvPortWolfSSLHeapInit()
{
	/*use 64k ccm memory*/
    my_mem_init(SRAMCCM);
}
