...
{
    return HAL_RTC_GetTime(&RTC_Handler, RTC_TimeStruct, RTC_FORMAT_BIN);
}

HAL_StatusTypeDef RTC_GetDate(RTC_DateTypeDef *RTC_DateStruct)
{
    return HAL_RTC_GetDate(&RTC_Handler, RTC_DateStruct, RTC_FORMAT_BIN);
}
...
struct tm* gmtime(const time_t* timer)
{
	...
	timenow.tm_year = date.Year+2000-1900;
	timenow.tm_mon  = date.Month;
	timenow.tm_mday = date.Date;
	
	timenow.tm_hour = time.Hours;
	timenow.tm_min  = time.Minutes;
	timenow.tm_sec  = time.Seconds;
